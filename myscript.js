...
function enforceCensorship() {
  ...
  chrome.storage.local.get(["enabled", "show_on_hover"], function(items) {
    ...
    if (disabled || enabled_everywhere == false) {
      ...
      $(".blur-it-out-always").removeClass("blur-it-out-always");
      return;
    } else {
      ...
      if (items["show_on_hover"] !== false) {
        ...
        $(".blur-it-out").removeClass("blur-it-out");
        $(".blur-it-out-always").not($ancestors).removeClass("blur-it-out-always");
        $ancestors.not(".blur-it-out-always").addClass("blur-it-out-always");
      }
    }
  });
}
