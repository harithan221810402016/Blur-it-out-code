...
$("#hover_behavior").click(function(){
  chrome.storage.local.get("show_on_hover", function(items){
    var enabled = items["show_on_hover"]
    if (enabled === undefined) {
      enabled = true
    }
    chrome.storage.local.set({"show_on_hover": !enabled}, rerender)
  })
})
...
function rerender() {
  var list = $("<ul/>");
  chrome.storage.local.get(["blacklist", "enabled", "badge_info", "show_on_hover"], function(items){
    ...
    if (items["enabled"] === false) {
      ...
      if (items["show_on_hover"] === false) {
        $("#hover_behavior").html("Enable hover");
      } else {
        $("#hover_behavior").html("Disable hover");
      }
